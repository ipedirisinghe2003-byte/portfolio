...
export class Animations {
  ...
  setupScrollAnimations(): void {
    ...
    getElements<HTMLElement>(
      '.project-card, .skill-category, .cert-card, .timeline-item, .mobile-app-card, .highlight-card'
    ).forEach((el) => {
      observer.observe(el);
    });
  }
   ...
   */
  setupTypewriterEffect(): void {
    const heroSubtitle = getElement<HTMLElement>('.hero-subtitle');
